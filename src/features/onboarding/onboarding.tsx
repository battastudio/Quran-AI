...
export function Onboarding() {
  ...
  const filtered = useMemo(() => (q ? reciters.filter((r) => r.name.includes(q)) : reciters), [q, reciters]);
    ...
    <div className="onboarding">
      <div className="onboarding__card">
        ...
        <label className="field">
          <span>اختر القارئ المفضّل ({arabicNum(reciters.length)} قارئ)</span>
          <input className="search-input" placeholder="ابحث عن قارئ…" value={q} onChange={(e) => setQ(e.target.value)} />
          <select size={5} value={reciter} onChange={(e) => set({ reciter: e.target.value })} className="reciter-select">
